...
def create_template_file(folder, residue_count, special_residues):
    ...
    for i, id in enumerate(template_id):
        if id == '3':
            # 检查前一个位置的元素
            if i > 0 and template_id[i-1] not in ['3', '4']:  # 确保i-1不会越界
                template_id[i-1] = '2'
            # 检查后一个位置的元素
            if i < len(template_id) - 1 and template_id[i+1] not in ['3', '4']:  # 确保i+1不会越界
                template_id[i+1] = '2'


    with open(template_path, 'w') as file:
        file.write('assembling\n  template_dirs {\n')
        file.write('    ../../../topo-fragments/outdir_martini\n')
        file.write('    ../../../topo-fragments/outdir_cg\n')
        file.write('    ../../../topo-fragments/outdir_aa\n')
        file.write('    ../LIG-prep\n  }\n  template_id {\n    ')
...
def main(base_directory, special_residues_str):
    ...
    if target_folder:
        ...
        pdb_file = os.path.join(target_folder, 'protein-lig-prep', "prot.pdb")
